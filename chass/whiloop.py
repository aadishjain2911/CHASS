...
def whiloop (thepassedfile, start_index, end_index ,index,variable_info,params):
	f = open(thepassedfile,"r+")
	g = open("whiloop" + str(index) + ".sh", "w+")
	lines = f.readlines()
	mylines = ""
	i=0
	...
	temporary.flush()
	subprocess.Popen(["bash","whiloop"+str(index)+".sh"]+params,stdout=temporary,stderr=subprocess.STDOUT)